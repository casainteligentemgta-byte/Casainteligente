...
| `ci_recepciones_campo` | FRM: ingreso provisional obra (nota / emergencia); `factura_canal_pendiente_id` al conciliar (migr. **199**) |
| `ci_recepciones_campo_lineas` | Líneas FRM → `global_inventory` + stock vía RPC |
...
2. `POST /api/almacen/recepcion/manual` → RPC `ci_registrar_ingreso_manual_campo` (migr. **199**) — stock atómico en `ubicacion_id`
3. Proveedor: `proveedor_nombre` (y opcional `proveedor_id` en `empresas`) para match con conciliación posterior
4. Cuando llega factura Telegram del mismo proveedor/obra → flujo **1** paso 8 (conciliar, no duplicar)

### 2. Recepción de mercancía (app)
1. `/almacen/procurement` — OCR opcional, líneas manuales; categoría **Consumibles / Logística de Campo** → partida operacional auto
...
GET    /api/almacen/recepcion/buscar-pendientes?proyecto_id=&proveedor_rif=
...
### Inventario + ubicaciones (180–185)
| # | Archivo |
|---|---------|
...
| 184 | `184_inv_despacho_alertas_proyecto.sql` — umbrales alertas despacho por obra |
| 185 | `185_ci_facturas_canal_idempotencia_telegram.sql` — `telegram_message_id` único + estado `recibido` |
| 198 | `198_add_limite_fast_track_proyectos.sql` — `ci_proyectos.limite_fast_track_usd` |
| 199 | `199_ci_recepciones_provisionales_campo.sql` — FRM + RPC `ci_registrar_ingreso_manual_campo` |
...
| Hydration failed (window) | Select value sin opción; patrón `montado` en despacho; `npm run clean:next` |
| Facturas Telegram duplicadas | Aplicar migr. **185** + índice `telegram_message_id` |
| Duplicados POST en iPad | `useSyncSubmitLock` en confirmación Telegram, FRM, conciliación, despacho |
| Compra confirmada sin stock | Revisar `extracted._ingreso_almacen_pendiente`; `POST .../ingreso-almacen` |
| Stock duplicado (nota + factura) | Conciliar FRM; no usar «Cargar compra» si tarjeta naranja activa |
| Fast-Track con FRM pendiente | Bloqueado en `evaluarFastTrackFactura` — conciliación manual |
...
- `lib/contabilidad/encolarIngresoAlmacenFallback.ts`

### Almacén / FRM
- `app/almacen/recepcion/RecepcionCampoClient.tsx`
- `lib/almacen/buscarRecepcionesCampoPendientes.ts`
- `lib/almacen/tieneRecepcionesCampoSinConciliar.ts`

### Telegram / canal
- `lib/telegram/webhook.ts`
- `lib/canal/processInvoiceFromCanal.ts`
- `lib/telegram/processInvoiceFromTelegram.ts`
- `lib/telegram/proyectoPicker.ts`
- `lib/telegram/ubicacionPicker.ts`
...
- `/api/contabilidad/compras/conciliar-frm`
- `/api/almacen/recepcion/buscar-pendientes`
- `/api/almacen/recepcion/manual`
...
## SUPER GEM V3 — IMPLACABLE (resumen operativo)

Los **5 pilares de blindaje** que todo cambio debe respetar:

1. **Idempotencia Telegram** — `recibido` + `telegram_message_id` + 23505 → 200 sin OCR duplicado; `reclamarProcesamientoFacturaCanal` antes de OCR.
2. **Techos Lulo en cliente** — `onChange` + `justificacion_gasto` antes del POST de transferencia.
3. **Anti-embudo partidas** — sugeridas → «Ver todas las partidas del presupuesto» automático si vacío.
4. **Consumibles / Logística de Campo** — literal exacto; `resolverPartidaConsumiblesCampo` (cap. operacional).
