...
2. `reclamarProcesamientoFacturaCanal` → un solo worker en `procesando`
3. Gemini OCR → `processInvoiceFromCanal.ts` → estado `extraido`
4. Picker inline **obra** (`proyectoPicker` modo `factura_compra`)
5. Picker **almacén** (`ubicacionPicker` callbacks `ub:` / `up:`)
6. Guarda `proyecto_id`, `ubicacion_destino_id` en pendiente
7. Link: `/contabilidad/compras/telegram/[id]` → `ConfirmarCompraTelegramClient`
8. `confirmarCompraDesdeCanal` → `purchase_invoices` + `contabilidad_compras` + ubicación

**Archivos:** `lib/telegram/webhook.ts`, `lib/canal/reservarFacturaCanalTelegram.ts`, `lib/canal/processInvoiceFromCanal.ts`, `lib/telegram/mediaHandlers.ts`, `lib/telegram/ubicacionPicker.ts`, `lib/contabilidad/confirmarCompraDesdeCanal.ts`
...
| Duplicados POST en iPad | `useSyncSubmitLock` en botones de confirmación |
...
1. **Idempotencia Telegram** — `recibido` + `telegram_message_id` + 23505 → 200 sin OCR duplicado.
...
**Checklist post-deploy:** migr. 180–185 en Supabase · reload schema · probar `/almacen/despacho` con obra Flamboyant · reenviar misma foto Telegram (no debe duplicar).

---

*Gem maestro v2026-05-28 — incluye inventario despacho por obra, blindaje campo e idempotencia Telegram (185).*
