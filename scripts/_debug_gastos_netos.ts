...
for (const r of rows) {
  ...
  if (c === 'GASTO') {
    gNet += base;
    if (base > 10000) {
      largeGastos.push({ desc: r.descripcion, base, orig: r.monto_orig, moneda: r.moneda, tasa: r.tasa });
    }
  }
}

console.log('gastosNetos:', gNet);
largeGastos.sort((a, b) => b.base - a.base);
console.log('Top 10 large gastos:', largeGastos.slice(0, 10));
