/**
 * Columna cargo en ci_telegram_whitelist (migración 231) + notify pgrst.
 * Uso: node scripts/apply-migration-231.mjs
 */
import fs from 'fs';
...
const migrationPath = path.join(root, 'supabase/migrations/231_ci_telegram_whitelist_cargo.sql');
...
async function main() {
  ...
  try {
    const yaExiste = await columnExists(sql, 'ci_telegram_whitelist', 'cargo');
    if (yaExiste) {
      console.log('✅ Migración 231 ya aplicada (columna cargo existe).');
      await sql.unsafe(`notify pgrst, 'reload schema';`);
      console.log('✅ PostgREST schema cache recargado.');
    } else {
      console.log('Aplicando 231_ci_telegram_whitelist_cargo.sql…');
      await sql.unsafe(sqlText);
      console.log('✅ Migración 231 aplicada.');
    }

    const filas = await sql`
      select nombre, chat_id, cargo, activo
      from public.ci_telegram_whitelist
      where activo = true
      order by nombre
    `;
    console.log(`\nWhitelist activa (${filas.length}):`);
    for (const f of filas) {
      const cargo = f.cargo ? ` · ${f.cargo}` : '';
      console.log(`  • ${f.nombre} (${f.chat_id})${cargo}`);
    }
  } finally {
