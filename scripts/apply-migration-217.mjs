/**
 * Repara ci_proyectos.proyecto_modulo_origen_id (migración 103 / 217).
 * Uso: node scripts/apply-migration-217.mjs
 */
import fs from 'fs';
...
const migrationPath = path.join(
  root,
  'supabase/migrations/217_ci_proyectos_proyecto_modulo_origen_repair.sql',
);
  ...
  try {
    const cols = await sql`
      ...
      where table_schema = 'public'
        ...
        and column_name = 'proyecto_modulo_origen_id'
    `;
    if (cols.length > 0) {
      console.log('✅ proyecto_modulo_origen_id ya existe; nada que hacer.');
      return;
    }

    console.log('Aplicando 217_ci_proyectos_proyecto_modulo_origen_repair.sql…');
    await sql.unsafe(sqlText);
    console.log('✅ Columna proyecto_modulo_origen_id creada.');
  } finally {
