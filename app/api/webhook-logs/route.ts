...
import { manejarCallbackSupervisorLogProcura } from '@/lib/procuras/supervisorLogBotProcura';
...
export async function POST(req: Request) {
  ...
  const admin = telegramSupabaseAdmin();
  if (admin.ok) {
    const handledSupervisor = await manejarCallbackSupervisorLogProcura(admin.client, {
      callbackId: cq.id,
      data: cq.data,
      chatId,
      messageId,
    ...
    if (handledSupervisor) {
      return NextResponse.json({ ok: true, action: 'supervisor_procura' });
    }
  }
