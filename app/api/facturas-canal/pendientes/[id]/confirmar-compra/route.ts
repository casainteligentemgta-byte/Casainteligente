...
export async function POST(req: Request, ctx: RouteCtx) {
  try {
    ...
    const gastoEntidad = body.imputacion_entidad === true;
    const proyectoId = String(body.proyecto_id ?? '').trim();
    const ubicacionDestinoId = String(body.ubicacion_destino_id ?? '').trim();
    const entidadId = String(body.entidad_id ?? '').trim();

    if (!gastoEntidad && !proyectoId) {
      return NextResponse.json({ error: 'Seleccione un proyecto.' }, { status: 400 });
    }
    if (gastoEntidad && !entidadId && !proyectoId) {
      return NextResponse.json({ error: 'Seleccione la entidad que absorbe el gasto.' }, { status: 400 });
    }
