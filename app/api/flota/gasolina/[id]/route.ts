...
import { actualizarGasolina, eliminarGasolina, obtenerGasolina } from '@/lib/flota/gasolina';
...
export async function GET(_req: Request, ctx: RouteCtx) {
  ...
  try {
    const { item, migracionPendiente } = await obtenerGasolina(auth.supabase, id);
...
export async function PUT(req: Request, ctx: RouteCtx) {
  ...
  try {
    const registro = await actualizarGasolina(auth.supabase, id, body);
    return NextResponse.json({ ok: true, registro });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Error al actualizar';
    return NextResponse.json({ error: msg }, { status: /requerido|inválid|mayor/i.test(msg) ? 400 : 500 });
  }
}
...
export async function DELETE(_req: Request, ctx: RouteCtx) {
  ...
  try {
    await eliminarGasolina(auth.supabase, id);
