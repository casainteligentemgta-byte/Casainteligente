...
export default function NuevoEmpleadoPage() {
    ...
    const [fotoMode, setFotoMode] = useState<'opciones' | 'archivo' | 'camara' | 'url'>('opciones');
    ...
    async function handleFotoFile(file: File) {
        ...
        try {
            ...
            const path = `empleados/${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`;
    ...
    async function handleSave() {
        ...
        const payload = {
            ...
            cargo, departamento, fecha_ingreso: fechaIngreso || null, salario: parseFloat(salario) || null,
            estatus, cuenta_bancaria: cuentaBancaria, banco, ivss,
            pretension_salarial: parseFloat(pretensionSalarial) || null, disponibilidad,
            ...
            tipo_sangre: tipoSangre, enfermedades, alergias, tratamientos,
            certificado_medico_grado: certMedicoGrado, certificado_medico_vencimiento: certMedicoVencimiento || null,
            vehiculo_propio: vehiculoPropio, vehiculo_marca: vehiculoMarca, vehiculo_anio: parseInt(vehiculoAnio) || null,
            licencia_grado: licenciaGrado, licencia_vencimiento: licenciaVencimiento || null,
            ...
            <div style={{ ...glass, padding: '28px', marginBottom: '24px' }}>
                ...
                {section === 0 && (
                    <div>
                        ...
                        <div style={rowStyle}>
                            <div><label style={labelStyle}>Apellidos *</label><input style={inputStyle} value={apellidos} onChange={e => setApellidos(e.target.value)} placeholder="Mata González" /></div>
                            <div><label style={labelStyle}>Nombres *</label><input style={inputStyle} value={nombres} onChange={e => setNombres(e.target.value)} placeholder="Luis Vicente" /></div>
                        </div>
                        <div style={rowStyle}>
                            <div><label style={labelStyle}>Cédula de Identidad *</label><input style={inputStyle} value={cedula} onChange={e => setCedula(e.target.value)} placeholder="V-13848186" /></div>
                            <div><label style={labelStyle}>RIF</label><input style={inputStyle} value={rif} onChange={e => setRif(e.target.value)} placeholder="J-407035258" /></div>
                        </div>
                        <div style={rowStyle}>
                        ...
                        <div style={{ marginBottom: '16px' }}><label style={labelStyle}>Dirección Actual</label><input style={inputStyle} value={direccion} onChange={e => setDireccion(e.target.value)} placeholder="Calle, Urb., Casa…" /></div>
                        <div style={rowStyle}>
                        ...
                        <div style={rowStyle}>
                            <div><label style={labelStyle}>Teléfono Habitación</label><input style={inputStyle} value={telHabitacion} onChange={e => setTelHabitacion(e.target.value)} placeholder="0295-…" /></div>
                            <div><label style={labelStyle}>Celular</label><input style={inputStyle} value={celular} onChange={e => setCelular(e.target.value)} placeholder="0412-…" /></div>
                        </div>
                        <div style={{ marginBottom: '16px' }}><label style={labelStyle}>Correo Electrónico</label><input type="email" style={inputStyle} value={email} onChange={e => setEmail(e.target.value)} placeholder="empleado@email.com" /></div>
                        {/* ── Foto del empleado ── */}
                        <div style={{ marginTop: '8px' }}>
                            ...
                            {fotoUrl && (
                                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '14px' }}>
                                    <div style={{ width: '72px', height: '72px', borderRadius: '50%', overflow: 'hidden', border: '2px solid rgba(0,174,239,0.4)', flexShrink: 0, position: 'relative' }}>
                                        <img src={fotoUrl} alt="preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                        {fotoUploading && (
                                            <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px' }}>⏳</div>
                                        )}
                                    </div>
                                    <div>
                                        <p style={{ margin: '0 0 4px 0', fontSize: '12px', color: fotoUploading ? '#FFD60A' : '#34C759', fontWeight: 700 }}>
                                            {fotoUploading ? 'Subiendo al servidor…' : '✅ Foto cargada'}
                                        </p>
                                        <button onClick={() => { setFotoUrl(''); setFotoMode('opciones'); }} style={{ padding: '5px 12px', borderRadius: '8px', border: '1px solid rgba(255,59,48,0.3)', background: 'rgba(255,59,48,0.08)', color: '#FF3B30', cursor: 'pointer', fontSize: '12px', fontFamily: 'inherit', fontWeight: 700 }}>
                                            🗑 Quitar foto
                                        </button>
                                    </div>
                                </div>
                            ...
                            {/* Selector de modo */}
                            {!fotoUrl && fotoMode === 'opciones' && (
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
                                    {[
                                        { mode: 'archivo', emoji: '📁', label: 'Desde galería' },
                                        { mode: 'camara',  emoji: '📷', label: 'Tomar foto'   },
                                        { mode: 'url',     emoji: '🔗', label: 'Pegar URL'    },
                                    ].map(opt => (
                                        <button key={opt.mode} onClick={() => {
                                            if (opt.mode === 'archivo') { fileInputRef.current?.click(); }
                                            else if (opt.mode === 'camara') { cameraInputRef.current?.click(); }
                                            else { setFotoMode('url'); }
                                        }} style={{ padding: '16px 10px', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.04)', color: 'white', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'center', transition: 'all 0.2s' }}>
                                            <div style={{ fontSize: '22px', marginBottom: '6px' }}>{opt.emoji}</div>
                                            <div style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(255,255,255,0.5)' }}>{opt.label}</div>
                                        </button>
                                    ))}
                                </div>
                            )}
                            ...
                            {/* URL manual */}
                            {fotoMode === 'url' && !fotoUrl && (
                                <div style={{ display: 'flex', gap: '10px' }}>
                                    <input
                                        type="url" autoFocus placeholder="https://ejemplo.com/foto.jpg"
                                        style={{ ...inputStyle, flex: 1 }}
                                        onKeyDown={e => { if (e.key === 'Enter') { setFotoUrl((e.target as HTMLInputElement).value); } }}
                                        onBlur={e => { if (e.target.value) setFotoUrl(e.target.value); }}
                                    />
                                    <button onClick={() => setFotoMode('opciones')} style={{ padding: '10px 14px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700, fontSize: '13px' }}>✕</button>
                                </div>
                            )}
                ...
                {section === 1 && (
                    <div>
                        ...
                        <div style={rowStyle}>
                            <div><label style={labelStyle}>Banco</label><input style={inputStyle} value={banco} onChange={e => setBanco(e.target.value)} placeholder="BANESCO…" /></div>
                    ...
                    <div>
                        ...
                        {idiomas.map((id, i) => (
                            <div key={i} style={rowStyle}>
                                <div><label style={labelStyle}>Idioma</label><input style={inputStyle} value={id.idioma} onChange={e=>{const n=[...idiomas];n[i].idioma=e.target.value;setIdiomas(n)}} placeholder="Inglés, Francés…" /></div>
                        ...
                        <div><label style={labelStyle}>Áreas Específicas de Conocimiento</label><textarea style={{ ...inputStyle, resize: 'vertical' }} rows={4} value={areasConocimiento} onChange={e => setAreasConocimiento(e.target.value)} placeholder="Proceso constructivo, instalaciones, redes…" /></div>
                    </div>
                )}
                ...
                {section === 6 && (
                    <div>
                        ...
                        <div style={rowStyle}>
                            ...
                            <div><label style={labelStyle}>Certificado Médico – Grado</label><input style={inputStyle} value={certMedicoGrado} onChange={e => setCertMedicoGrado(e.target.value)} placeholder="1er grado, 2do grado…" /></div>
                        </div>
                        ...
                        <div style={{ marginBottom: '16px' }}><label style={labelStyle}>Enfermedades Crónicas</label><textarea style={{ ...inputStyle, resize: 'vertical' }} rows={3} value={enfermedades} onChange={e => setEnfermedades(e.target.value)} placeholder="Hipertensión, diabetes…" /></div>
                ...
                {section === 7 && (
                    <div>
                        ...
                        <div style={rowStyle}>
                            <div><label style={labelStyle}>Grado</label><input style={inputStyle} value={licenciaGrado} onChange={e => setLicenciaGrado(e.target.value)} placeholder="3er grado…" /></div>
                        ...
                        <div><label style={labelStyle}>Medio de Transporte Utilizado</label><input style={inputStyle} value={medioTransporte} onChange={e => setMedioTransporte(e.target.value)} placeholder="Propio, Metro, Bus, Moto…" /></div>
                    </div>
                )}
                    ...
                    <div>
                        ...
                        {referencias.map((ref, i) => (
                            <div key={i} style={{ marginBottom: '16px', padding: '16px', background: 'rgba(255,255,255,0.03)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.06)' }}>
                                <div style={rowStyle}>
                                    <div><label style={labelStyle}>Nexo (Familiar, Amigo…)</label><input style={inputStyle} value={ref.nexo} onChange={e=>{const n=[...referencias];n[i].nexo=e.target.value;setReferencias(n)}} /></div>
