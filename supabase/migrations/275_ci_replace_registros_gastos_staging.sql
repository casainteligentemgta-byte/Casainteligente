-- Reemplazo limpio transaccional del libro CCO (CSV diario acumulado).
-- Flujo: cargar lotes en staging → COMMIT (TRUNCATE destino + INSERT desde staging) en una sola TX.
-- Así reimportar el mismo CSV deja el mismo conteo (no duplica).
...
COMMENT ON TABLE public.registros_gastos_staging IS
  'Staging temporal para import CSV diario → registros_gastos (reemplazo atómico).';
...
BEGIN
  ...
  -- Reemplazo atómico: el destino solo cambia si todo el INSERT tiene éxito.
...
-- Lectura/escritura staging vía service role (API admin); RLS opcional.
