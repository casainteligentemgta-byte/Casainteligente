-- FRM: forma de ingreso y soporte fotográfico por línea (auditoría / conciliación factura tardía).

alter table public.ci_recepciones_campo_lineas
  add column if not exists forma_ingreso varchar(32) not null default 'sin_nota'
    check (forma_ingreso in ('con_factura', 'con_nota', 'sin_nota', 'pendiente_factura'));

alter table public.ci_recepciones_campo_lineas
  add column if not exists soporte_storage_path text;

alter table public.ci_recepciones_campo_lineas
  add column if not exists soporte_file_name text;

alter table public.ci_recepciones_campo_lineas
  add column if not exists soporte_mime_type text;

comment on column public.ci_recepciones_campo_lineas.forma_ingreso is
  'Tipo documental de la línea: con factura, nota, sin nota o factura pendiente (conciliar después).';

comment on column public.ci_recepciones_campo_lineas.soporte_storage_path is
  'Foto de soporte de esta línea (nota parcial, material recibido, etc.).';

create index if not exists idx_ci_recepciones_campo_lineas_forma
  on public.ci_recepciones_campo_lineas (forma_ingreso);
...
comment on function public.ci_registrar_ingreso_manual_campo is
  'FRM: recepción provisional con forma_ingreso y soporte fotográfico por línea.';
