...
export default function NetVisionProjectsPanel({
  ...
  return (
    <div className="relative">
      ...
      {open ? (
        <div className="absolute right-0 z-40 mt-2 w-[min(100vw-2rem,380px)] rounded-xl border border-white/15 bg-[#0d1118] p-3 shadow-xl shadow-black/50">
          ...
          <ul className="mb-3 max-h-40 space-y-1 overflow-auto">
            {entries.length === 0 ? (
              <li className="text-xs text-[var(--nexus-text-dim)]">Sin proyectos locales.</li>
            ) : (
              entries.map((e) => {
