...
export type DestinoAlmacenFacturaResuelto = {
...
/** Obra / entidad contable a partir del almacén elegido (incl. depósitos sin ci_proyecto_id). */
export async function resolverProyectoDestinoDesdeUbicacionAlmacen(
  supabase: SupabaseClient,
  ubicacionId: string,
): Promise<DestinoAlmacenFacturaResuelto> {
  const uid = ubicacionId.trim();
  if (!uid) throw new Error('Ubicación no indicada.');
  ...
  const { data: ubi, error } = await supabase
    ...
    .select(
      ...
      deposit:inventory_deposits ( id, name, entidad_id )
    `,
    )
    .eq('id', uid)
    .maybeSingle();
  ...
  const proyRaw = ubi.ci_proyecto_id ?? (Array.isArray(ubi.proyecto) ? ubi.proyecto[0] : ubi.proyecto);
  const proy = proyRaw && typeof proyRaw === 'object' ? proyRaw : null;
  const depRaw = Array.isArray(ubi.deposit) ? ubi.deposit[0] : ubi.deposit;
  const dep = depRaw && typeof depRaw === 'object' ? depRaw : null;

  const ubicacionNombre = String(ubi.nombre ?? 'Almacén').trim() || 'Almacén';

  if (proy?.id) {
    return {
      proyectoId: String(proy.id),
      entidadId: String(proy.entidad_id ?? '').trim() || null,
      imputacionEntidad: false,
      nombreObra: String(proy.nombre ?? 'Obra').trim() || 'Obra',
      ubicacionNombre,
    };
  }

  const entidadId = String(dep?.entidad_id ?? '').trim() || null;
  if (!entidadId) {
    throw new Error(
      'Este almacén no está vinculado a una obra ni a un patrono. Configúrelo en Almacén → Maestros.',
    );
  }
  ...
  const { data: opex } = await supabase
    ...
    .eq('naturaleza_proyecto', 'centro_costo_entidad')
    .order('nombre')
    .limit(1)
    .maybeSingle();
  ...
  if (opex?.id) {
    return {
      proyectoId: String(opex.id),
      entidadId,
      imputacionEntidad: false,
      nombreObra: String(opex.nombre ?? dep?.name ?? ubicacionNombre).trim() || ubicacionNombre,
      ubicacionNombre,
    };
  }
  ...
  return {
    ...
    nombreObra: String(dep?.name ?? ubicacionNombre).trim() || ubicacionNombre,
    ubicacionNombre,
  };
}
