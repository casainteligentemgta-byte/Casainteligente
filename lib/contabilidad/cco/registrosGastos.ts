...
export function gastoRegistroALibroFila(r: GastoRegistro): CcoLibroFila {
  ...
  return {
    id: String(r.id),
    display_id: Number.isFinite(idNum) ? idNum : String(r.id),
    origen_v4_id: null,
    clase,
    ...
    admin_pct: num(r.porcentaje_admin),
    ...
    tiene_documento: Boolean(r.link_factura || r.link_comprobante),
    document_file_name: null,
    monto_pagado_usd: r.monto_pagado != null ? num(r.monto_pagado) : null,
    tasa_binance: num(r.tasa_binance),
    tasa_usada: r.tasa_usada,
    porcentaje_brecha_real: r.porcentaje_brecha_real,
    contrato_label: r.contrato_vinculado,
