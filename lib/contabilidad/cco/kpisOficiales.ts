...
export type CcoKpisOficiales = {
  ...
  /**
   * Devaluación V4 (típicamente negativa) derivada del promedio de brechas CSV.
   * Si no hay brechas, 0.
   */
...
export function calcularKpisOficiales(opts: {
  ...
  const brechaRaw = nBrecha > 0 ? sumaBrecha / nBrecha : 0;
  const devaluacionPromedioBrechas = brechaRaw !== 0 ? brechaCsvADevaluacionV4(brechaRaw) : 0;
