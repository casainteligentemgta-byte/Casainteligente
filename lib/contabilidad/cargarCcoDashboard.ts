...
export async function cargarCcoDashboard(
  ...
  const selectComprasBase =
    'id,fecha,proyecto_id,monto_usd,monto_ves,total_amount,imputacion,supplier_name,tasa_bcv_ves_por_usd,moneda_original';
  ...
  // Override del query param gana; si no, config de obra (>0); si no, promedio de brechas V4.
  const devaluacionPromedioBase = devaluacionOverride ?? devaluacionDesdeConfig ?? 0;
  ...
  const gastosKpiInput = (compras ?? []).map((row) => row as CompraRow);
